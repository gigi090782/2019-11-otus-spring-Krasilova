package ru.krasilova.otus.spring.homework7.shell;
...
import ru.krasilova.otus.spring.homework7.service.IOService;
import ru.krasilova.otus.spring.homework7.service.LibraryService;
...
public class ShellLibraryCommands {
    ...
    public void showAllCommentsByBookId( @ShellOption(defaultValue = "1") Long bookId) {
    ...
    public void addComment(@ShellOption(defaultValue = "Новый комментарий") String commentName,
                           @ShellOption(defaultValue = "1") Long bookId) {
        try {
            boolean result = libraryService.addNewComment(commentName, bookId);
            if (result)
                libraryService.showAllCommentsByBookId(bookId);
        } catch (Exception e) {
    ...
    public void deleteComment( @ShellOption(defaultValue = "1") Long commentId) {
        try {
            boolean result = libraryService.deleteComment(commentId);
            if (result)
                libraryService.showAllComments();
        } catch (Exception e) {
            ioService.printString("Ошибка в удалении комментария: " + e.getMessage());
        }
    }
}
